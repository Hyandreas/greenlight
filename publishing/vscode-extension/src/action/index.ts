...
async function run(): Promise<void> {
  try {
    ...
    const configPath = core.getInput('config') || './baseline.config.json';
...
function generateOutput(results: CompatibilityResult[], format: string): string {
  ...
  if (format === 'sarif') {
    filename = 'baseline-results.sarif';
  } else {
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
    filename = `baseline-results-${timestamp}.${format === 'markdown' ? 'md' : 'json'}`;
  }
...
async function commentOnPR(results: CompatibilityResult[], outputFile: string, token: string): Promise<void> {
  try {
    ...
    if (existsSync(outputFile) && outputFile.endsWith('.md')) {
      ...
      if (results.length === 0) {
        comment = '✅ **Baseline Compatibility**: No issues found!';
      } else {
        comment = `🚨 **Baseline Compatibility**: Found ${results.length} issues (${errorCount} errors, ${warningCount} warnings)`;
      }
    }
