...
export class ConfigLoader {
  ...
  constructor() {
    ...
    addFormats(this.ajv);
    this.ajv.addSchema(baselineConfigSchema, 'greenlight-config');
  }

  /**
   * Load configuration from greenlight.config.json or return defaults
   */
  loadConfig(workspaceRoot?: string, configPath?: string): BaselineConfig {
    ...
    try {
      ...
      if (finalConfigPath.endsWith('package.json')) {
        userConfig = userConfig.greenlight || {};
      }
  ...
  private findConfigFile(workspaceRoot?: string): string | null {
    const configFiles = [
      ...
      '.greenlight.json',
      'package.json' // Can embed config in package.json under "greenlight" field
    ];
    ...
    for (const searchPath of searchPaths) {
      for (const configFile of configFiles) {
        ...
        if (existsSync(fullPath)) {
          // For package.json, check if it has greenlight config
  ...
  private validateConfigSchema(config: any, configPath: string): void {
    const validate = this.ajv.getSchema('greenlight-config');
    if (!validate) {
