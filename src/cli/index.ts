...
program
  ...
  .option('-c, --config <path>', 'Configuration file path', 'greenlight.config.json')
    ...
    try {
      ...
      switch (options.format) {
        case 'markdown':
          ...
          fileName = options.output || 'greenlight-results.md';
          break;
        case 'sarif':
          output = generateSarifReport(results);
          fileName = options.output || 'greenlight-results.sarif';
          break;
        case 'json':
          output = JSON.stringify(results, null, 2);
          fileName = options.output || 'greenlight-results.json';
          break;
        case 'console':
        default:
